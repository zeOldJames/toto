 ...
 * the License.
 */
package com.github.framework3eclipse.utils;
...
public final class Utils {
	 ...
	 * @throws JavaModelException
	 */
	public static boolean fieldIsPrimitiv(IType type, String fieldName) throws JavaModelException {
		IField fields[] = type.getFields();
		for (int i = 0; i < fields.length; i++) {
			if (fieldName.equals(fields[i].getElementName())) {
				String typeSignature = fields[i].getTypeSignature();
				if (typeSignature.endsWith(Signature.SIG_BOOLEAN) || typeSignature.endsWith(Signature.SIG_BYTE)
						|| typeSignature.endsWith(Signature.SIG_CHAR) || typeSignature.endsWith(Signature.SIG_DOUBLE)
						|| typeSignature.endsWith(Signature.SIG_FLOAT) || typeSignature.endsWith(Signature.SIG_INT)
						|| typeSignature.endsWith(Signature.SIG_LONG) || typeSignature.endsWith(Signature.SIG_SHORT)) {
					return true;
				}
			}
		}
	...
	public static String getBoxingString(IType type, String fieldName) throws JavaModelException {
		IField fields[] = type.getFields();
		for (int i = 0; i < fields.length; i++) {
			if (fieldName.equals(fields[i].getElementName())) {
				String typeSignature = fields[i].getTypeSignature();
